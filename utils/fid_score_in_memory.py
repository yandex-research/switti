...
def get_activations(
    ...
    dataset = ImageDataset(
        ...
        transforms=TF.Compose(
            [
                TF.Resize(resolution, interpolation=TF.InterpolationMode.LANCZOS),
...
def compute_statistics_of_path(
    ...
    if path.endswith(".npz"):
        ...
        m, s = calculate_activation_statistics(
            images, model, batch_size, dims, device, num_workers, resolution
        )
...
def save_statistics_of_path(
    ...
    m1, s1 = compute_statistics_of_path(
        path, model, batch_size, dims, device, num_workers, resolution
    )
